...
One tap, from the home screen or the iOS lock screen, that opens the app with
the seizure timer already running. Both platforms open the same URL:

```
pawtrack://seizure/start   ->  app/seizure/start.tsx
```

That route is the whole integration. A widget can only hand the app a URL, and
`/seizure/live` **shows** a timer rather than starting one — so linking there
directly would produce a screen counting from the moment it mounted, with no
row in SQLite behind it. `start.tsx` does what the in-app Record buttons do
(`startSeizure(dog.id)` then navigate) and replaces itself.

**Both halves are generated at prebuild time, and that is not optional.**
`/ios` and `/android` are gitignored build output in this project. Native code
committed into either survives exactly until the next `expo prebuild --clean`,
and then vanishes from every owner's home screen with nothing in the diff to
explain it.

| | Source | Generated by |
|---|---|---|
| iOS | `targets/seizure/` (SwiftUI) | `@bacons/apple-targets` |
| Android | `plugins/withSeizureWidget.js` (Kotlin + res XML) | its own dangerous mod |

iOS supports `systemSmall` plus the `accessoryCircular` and
`accessoryRectangular` lock-screen families — the lock screen being the single
most valuable placement, since it is reachable without unlocking the phone.
Android is a fixed 2×2 tile.

**Neither works in Expo Go.** A widget is native code outside the JS bundle:
...
iOS additionally needs a signing team for the extension target — set
`APPLE_TEAM_ID` in `.env`, which `app.config.ts` reads into `ios.appleTeamId`.
Without it the app target builds and the widget target fails on its own, which
reads as a broken build rather than a missing setting.

### What it deliberately does not do

Show the elapsed time of a seizure in progress. That needs an App Group (iOS)
or a pushed `RemoteViews` update (Android) on every tick, and both platforms
are free to throttle the refresh. A timer that is sometimes wrong is worse on
this screen than no timer at all — the live screen owns elapsed time.

It also cannot recover the seconds a cold start costs. Both platforms bake the
tap URL in when the widget is **rendered**, not when it is tapped, so a
timestamp in the URL would be whatever the last refresh happened to be. The
live screen's start-time adjustment is the honest fix.
...
app/                       Routes (Expo Router owns this folder)
  ...
  seizure/                 live -> post -> recovery emergency flow
    start.tsx              widget deep-link landing; starts the timer
...
plugins/                   Config plugins — native config that app.config.ts
                           cannot express (Android widget, permission caps)
targets/seizure/           The iOS widget, linked into Xcode at prebuild
...
still works completely — every screen keeps reading local SQLite.

An account, however, is **required**. That is a change: accounts used to be
optional and the sign-in screen could be declined forever. Signing in is now
the first thing the app does, because records are stored against a user and
the agreement to the Terms and Privacy Policy is recorded against that same
user (`src/services/consent.ts`), so there is no state the app can be in before
one exists. This is about IDENTITY, not connectivity — once signed in, the app
is as offline-capable as it ever was.
